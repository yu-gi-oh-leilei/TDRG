...
# try:
#     from .trans_utils_tdrg.position_encoding import build_position_encoding
#     from .trans_utils_tdrg.transformer import build_transformer
# except ImportError:
#     from trans_utils_tdrg.position_encoding import build_position_encoding
#     from trans_utils_tdrg.transformer import build_transformer
...
class TDRG(nn.Module):
    ...
    def get_config_optim(self, lr, lrp):
        small_lr_layers = list(map(id, self.layer1.parameters())) + \
                          list(map(id, self.layer2.parameters())) + \
                          list(map(id, self.layer3.parameters())) + \
                          list(map(id, self.layer4.parameters()))
        large_lr_layers = filter(lambda p:id(p) not in small_lr_layers, self.parameters())
        return [
                {'params': self.layer1.parameters(), 'lr': lr * lrp},
                {'params': self.layer2.parameters(), 'lr': lr * lrp},
                {'params': self.layer3.parameters(), 'lr': lr * lrp},
                {'params': self.layer4.parameters(), 'lr': lr * lrp},
                # {'params': small_lr_layers, 'lr': lr*lrp},
                # {'params': nn.ModuleList([self.layer1, self.layer2, self.layer3, self.layer4]).parameters(), 'lr': lr * lrp},
